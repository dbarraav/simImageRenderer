...
output_frequency = 40
render_dirs = ['oldFigs', 'newFigs']
screenshotFiles = ['old_screenshots.json', 'new_screenshots.json']
...
def main():
    ...
    for num, jsonFile in enumerate(screenshotFiles):
        screenshot_spec = join(sim_dir, 'screenshot_data', jsonFile)  # screenshot json
        print(screenshot_spec)
        # Render results and store the renders in a specific location
        render_dir_abs = join(dirname(__file__), render_dirs[num])  # Storing here
